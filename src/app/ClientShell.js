        ...
        >
          <div className="w-11 h-11 bg-white rounded-md flex items-center justify-center p-1 shadow-sm">
            ...
            >
              ...
              <path
                d="M50 20 L20 60 L50 100 L80 60 Z"
                fill="none"
                stroke="#000"
                strokeWidth="4"
              />
          ...
          <div className="flex flex-col hidden sm:flex">
            <span className="font-black text-base text-text-inverse tracking-tight leading-none">
...
function BackendWakeupLoader({ children }) {
  ...
  const [phase, setPhase] = useState("init"); // 'init' | 'waking' | 'warming'

  useEffect(() => {
    let mounted = true;
    let retryTimer;

    const pingWake = async () => {
      try {
        // /api/wake proxy has maxDuration=60 and waits up to 58s for Render to respond.
        // It also triggers background PGVector warm-up on the backend.
        const res = await fetch("/api/wake");
        if (mounted) {
          if (res.ok) {
            setIsAwake(true);
          } else {
            // 503 means the proxy itself timed out (Render still cold)
            setPhase("waking");
            retryTimer = setTimeout(pingWake, 3000);
          }
        }
      } catch {
        if (mounted) {
    ...
    // Brief delay so the initial render shows "Initializing" before the network call
    retryTimer = setTimeout(pingWake, 300);
    return () => {
      mounted = false;
  ...
  const messages = {
    ...
    waking: "Waking up cloud server — this takes ~50 s on first visit...",
    warming: "Server is up! Loading AI engine...",
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-64px)] p-6">
      ...
      <div className="text-center max-w-md bg-bg-surface p-8 rounded-3xl border-2 border-border-strong shadow-lg">
        ...
        <p className="text-sm font-bold text-accent mb-4 animate-pulse">
          {messages[phase]}
        </p>
        <p className="text-xs text-text-muted leading-relaxed">
          The backend is hosted on Render&apos;s Free Tier, which automatically
          pauses after 15 minutes of inactivity.
          <br />
          <br />
          <strong>It takes about 50 seconds to boot back up.</strong> Please
          hold on!
        </p>
      </div>
    </div>
  ...
  return (
    <AuthProvider>
      ...
      <main className="bg-bg-base min-h-[calc(100vh-64px)] transition-colors duration-300">
        <BackendWakeupLoader>{children}</BackendWakeupLoader>
      </main>
    </AuthProvider>
  );
}
