      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (res.ok) {
            ...
            setModalAlert({ title: "Deleted", message: `"${doc.filename}" has been removed.`, type: "info" });
          } else {
            setModalAlert({ title: "Delete Failed", message: body.detail || `Server error (${res.status})`, type: "error" });
          }
        } catch (e) {
          setModalAlert({ title: "Error", message: `Network error: ${e.message}`, type: "error" });
        } finally {
          setDeleting(null);
        }
      }
  ...
  const handleView = async (doc, edit = false) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDocText(d.extracted_text || "No text content stored for this document.");
      } else {
        const body = await res.json().catch(() => ({}));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setViewingDoc(null);
        fetchDocuments();
        setModalAlert({ title: "Saved", message: "Document text updated and AI re-scan queued.", type: "info" });
      } else {
        setModalAlert({ title: "Save Failed", message: body.detail || `Server error (${res.status})`, type: "error" });
      }
    } catch (e) {
      setModalAlert({ title: "Error", message: `Network error: ${e.message}`, type: "error" });
    } finally {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setModalAlert({ title: "Sync Failed", message: "Failed to sync files from backend.", type: "error" });
      }
    } catch (e) {
      setModalAlert({ title: "Error", message: `Network error: ${e.message}`, type: "error" });
    } finally {
    ...
    <div className="p-8 space-y-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-bg-base border-4 border-dashed border-red-500 rounded-3xl">
          ...
          <p className="text-sm font-bold text-text-muted mt-2 px-4">{errorState}</p>
          <button onClick={fetchDocuments} className="mt-4 px-4 py-2 bg-primary text-white rounded-xl text-sm font-bold">Retry</button>
        </div>
      ) : documents.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {documents.map((doc) => {
            ...
            return (
              <div key={doc.id} className={`flex items-center gap-4 bg-bg-base border-2 rounded-2xl p-5 hover:shadow-md transition-all group ${riskBadge ? 'border-red-400' : 'border-border-strong hover:border-accent'}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    {doc.risk_level && (
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-black uppercase border shadow-sm ${
                        ...
                        doc.risk_level === 'Error' ? 'bg-red-100 text-red-800 border-red-300' :
                        ...
                        {doc.risk_level === 'Scanning...' ? '⏳ Scanning' : 
                         ...
                         doc.risk_level === 'Error' ? '❌ Scan Failed' : 
                         `⚠ ${doc.risk_level} Risk`}
                      </span>
                    )}
                  ...
                  {doc.risk_level && doc.risk_level !== 'None' && doc.risk_level !== 'Scanning...' && doc.risk_description && (
                    <p className="text-[11px] text-red-600 mt-1.5 font-semibold line-clamp-1 italic">{doc.risk_description}</p>
                  )}
                </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between pt-4 border-t border-border-strong">
          ...
          <div className="flex gap-3">
            <button disabled={page === 0} onClick={() => setPage(p => p - 1)} className="px-5 py-2.5 text-sm font-black rounded-xl border-2 border-border-strong text-text-main bg-bg-base hover:bg-bg-surface disabled:opacity-40 shadow-sm transition-colors">
              ← Prev
            </button>
            <button disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} className="px-5 py-2.5 text-sm font-black rounded-xl border-2 border-border-strong text-text-main bg-bg-base hover:bg-bg-surface disabled:opacity-40 shadow-sm transition-colors">
              Next →
            </button>
          </div>
        </div>
      ...
      {viewingDoc && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 backdrop-blur-md p-4 overflow-y-auto">
          <div className={`bg-bg-base rounded-3xl shadow-2xl border-2 border-border-strong w-full ${isEditing ? 'max-w-3xl' : 'max-w-6xl'} flex flex-col my-auto`}>
            ...
            <div className="flex justify-between items-start px-8 py-6 border-b border-border-strong shrink-0 bg-bg-surface rounded-t-3xl">
              <div>
                <div className="flex items-center gap-3">
                  ...
                  {viewingDoc.risk_level && viewingDoc.risk_level !== 'None' && (
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-black uppercase border shadow-sm ${
                      viewingDoc.risk_level === 'High' ? 'bg-red-100 text-red-800 border-red-300' :
                ...
                {viewingDoc.risk_description && viewingDoc.risk_level !== 'None' && viewingDoc.risk_level !== 'Scanning...' && (
                  <p className="text-xs text-red-500 font-semibold mt-1 italic">{viewingDoc.risk_description}</p>
                )}
              ...
              <button onClick={() => setViewingDoc(null)} className="w-10 h-10 rounded-full border-2 border-border-strong flex items-center justify-center text-text-main hover:bg-bg-base transition-colors shadow-sm shrink-0 ml-4">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>

            {/* Modal Body — scrollable */}
            <div className={`p-8 flex flex-col gap-6 ${!isEditing ? 'md:flex-row' : ''}`}>
              {!isEditing && (
                <div className="w-full md:w-1/2 bg-bg-surface border-2 border-border-strong rounded-3xl overflow-hidden shadow-inner" style={{ minHeight: '300px', maxHeight: '500px' }}>
                  {viewingDoc.contentType?.startsWith('image/') ? (
                    ...
                    <iframe src={`/api/documents/${viewingDoc.id}/file?token=${token}`} className="w-full h-full border-0" style={{ minHeight: '300px' }} />
                  ) : (
                    <div className="flex-1 w-full p-8 overflow-y-auto bg-white text-black text-sm leading-relaxed" style={{ maxHeight: '500px' }}>
                      <div className="mb-4 pb-4 border-b border-gray-200">
                        <h1 className="text-xl font-bold mb-1 font-sans">{viewingDoc.filename}</h1>
                      ...
                      <div className="whitespace-pre-wrap font-serif text-gray-800">
                        {docLoading ? 'Loading…' : (docText || 'No text content.')}
                      </div>
                    </div>
                  )}
                </div>
              ...
              {/* Text panel */}
              <div className={`${!isEditing ? 'w-full md:w-1/2' : 'w-full'} flex flex-col gap-3`}>
                <label className="text-sm font-black text-text-main uppercase tracking-widest">Extracted Text Content</label>
                {docLoading ? (
                  <div className="flex items-center justify-center p-12 border-2 border-border-strong rounded-2xl bg-bg-base">
                    ...
                    <span className="text-text-muted font-bold text-sm">Loading from backend…</span>
                  </div>
                ) : isEditing ? (
                  ...
                  <div className="w-full p-6 text-sm text-text-main bg-bg-base border-2 border-border-strong rounded-2xl overflow-y-auto whitespace-pre-wrap font-medium leading-relaxed shadow-inner" style={{ maxHeight: '400px' }}>
                    {docText}
                  </div>
                )}
              </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
          <div className="bg-bg-base border-2 border-border-strong rounded-3xl shadow-2xl w-full max-w-sm p-8 space-y-6">
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mx-auto border-2 shadow-md ${
              ...
              {modalAlert.type === 'confirm' && <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>}
